...
public class Config {
	...
	public String onTickMethodOrderString() {
		...
		for (OnTickMethod method : onTickMethodOrder) {
			...
			switch (method) {
				...
				case RIFT:
					result += Constants.ONTICKMETHOD_RIFT_NAME;
					break;
			}
		}
		return result;
	...
	public void onTickMethodOrderFromString(String string) {
		...
		for (String method : onTickMethods) {
			...
			else if (Constants.ONTICKMETHOD_RIFT_NAME.equalsIgnoreCase(method))
				onTickMethodOrder.add(OnTickMethod.RIFT);
		}
	...
	static {
		...
		defaultValues.setProperty(Constants.CONFIG_ONTICK_METHOD_ORDER, "Forge, LiteLoader, Rift");
